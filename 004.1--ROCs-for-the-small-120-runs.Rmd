...
roc_page <- function(mig_rate, samp_ppn, x, outfolder = "outputs/004", row_facet = "miss_rate") {
  ...
  if(row_facet == "miss_rate") {
    ...
    g2 <- g + facet_grid(
      mig_rate1 ~ scenario + cohort_inclusion + miss_rate,
      labeller = labeller(
        mig_rate1 = function(x) paste0(as.numeric(x) * 100, "%"),
        miss_rate = function(x) paste0(as.numeric(x) * 100, "%")
      )
    )
...
Now, we are going to unnest the whole thing, and only keep the exclude_same_cohort,
and rename some things.  This is sort of messy getting things to work in the
original framework for plotting stuff.  Hacky...
```{r}
unnesto <- nestoSR %>% 
  unnest(cols = data) %>%
  filter(cohort_inclusion == "exclude_same_cohort") %>%
  mutate(cohort_inclusion = "Exclude Same Cohort") %>%
  mutate(
    scenario = case_when(
      scenario == "small_cyclone_WF" ~ "Wright-Fisher (120)",
      scenario == "small_cyclone_nonWF" ~ "non-WF (120)",
    )
  )
...
roc_page(
  ...
  samp_ppn = 0.5,
  x = unnesto,
  outfolder = "outputs/004/SmallRun-120",
  row_facet = "mig_rate"
)
