...
nesto <- bind_rows(BR, RA) %>%
  ...
  mutate(mig_rate1 = mig_rate1, mig_rate2, samp_ppn) %>%
  nest()
...
categ_tib <- tribble(
  ...
  "Naive (v only, no FS calc)" ,  "Naive LR (V)"       , "None"                 ,
  "Naive (v only, w/FS calc)"  ,  "Naive LR (V)"       , "Likely FS Removed"    ,
  "Naive (v+d, no FS calc)"    ,  "Naive LR (V+D)"     , "None"                 ,
  "Naive (v+d, wFS calc)"      ,  "Naive LR (V+D)"     , "Likely FS Removed"    ,
...
fs_line_types <- c(
  None = "solid",
  `Likely FS Removed` = "dotted"
)

roc_page <- function(mig_rate, samp_ppn, x, outfolder = "outputs/004", row_facet = "miss_rate", make_title = FALSE) {

  x2 <- x %>%
    mutate(
      method = replace_na(method, "sequoia"),
      full_sib_treatment = replace_na(full_sib_treatment, "none"),
      miss_rate = var_miss_rate * 100
    ) %>%
    mutate(
      approach = case_when(
    ...
    mutate(
      cohort_inclusion = case_match(
        cohort_inclusion,
        "exclude_same_cohort" ~ "Exclude Same Cohort",
        "include_same_cohort" ~ "Include Same Cohort"
      ),
      scenario = case_match(
        scenario,
        "cyclone_WF" ~ "Wright-Fisher",
        "cyclone_nonWF" ~ "non-WF"
      )
    ) %>%
  ...
  g <- x2 %>%
    ...
    #theme_bw() +
    #ggtitle(str_c("Migration Rate: ", mig_rate, "   Sampled Fraction: ", samp_ppn)) +
    ylab("Fraction of All True Parent Offspring Pairs Correctly Identified (TPR)") +
    xlab("Fraction of the Parents of All Candidate Offspring Incorrectly Identified (FPR)")
...
BOING <- nesto %>% 
  mutate(
    plot = pmap_chr(
      .l = list(mig_rate = mig_rate1, samp_ppn = samp_ppn, x = data, make_title = TRUE),
      .f = roc_page,
      .progress = TRUE
    )
  )
...
roc_page("variable.  Fraction of Markers Randomly Missing = 0.25.", "0.5.", tmp, row_facet = "mig_rate")
```
...
roc_page("0.02",  "variable.  Fraction of Markers Randomly Missing = 0.25.", tmp2, row_facet = "samp_ppn")
```
...
palette = c(
    ...
    "#D55E00", "#F0E442",  # Vermilion/Orange
