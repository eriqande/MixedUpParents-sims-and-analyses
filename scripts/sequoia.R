...
if(exists("snakemake")) {
  ...
  slim_inrds <- "development/small-langford-slim-output.rds"
  seq_results <- "sequoia_res.rds"
}
...
true_trios_sexes.tib$LLRpair <- NA
...
for (offsp_result in 1:nrow(true_trios_sexes.tib)) {
  ...
  if (!is.na(sim_result_dam)) {
    ...
    if (true_dam == sim_result_dam) {
      ...
      true_trios_sexes.tib$LLRpair[offsp_result] <- sim_result_triad$LLRpair
